...
obstacle3 = [-1.2, 4,-4.5 , -3.5, 0, 5];      %[vert3, fac3]   = vertObstacle(obstacle3);
obstacle3_1= [-1.2, 4,-4.25 , -3.75, 0, 5]; [vert3, fac3]   = vertObstacle(obstacle3_1);
obstacle4 = [-4 , 0, 3.05, 3.25 , 0, 5];      [vert4, fac4]   = vertObstacle(obstacle4);
obstacle5 = [-0.175, 0.025, 0.04, 3.04, 0, 5];      [vert5, fac5]   = vertObstacle(obstacle5);
...
for t = 1:Param.delta:Param.LIM
   ...
   xlim([-8 8]); ylim([-9 9]); zlim([0 5]);
    ...
    % -----------------------------------------------------------
    % 保存先フォルダの設定
    save_dir = 'C:\Users\Murakami\村上\先輩いじる\プログラム\未知障害物と既知障害物\figure';
    ...
    if abs(rem(current_time, 1.5)) < Param.delta / 2
        ...
        fname = sprintf('simv2_t_%04.1f.png', current_time);
...
%% アニメーション書き出し用の処理
Frate = 20;
v = VideoWriter('C:\Users\Murakami\村上\先輩いじる\プログラム\未知障害物と既知障害物\figure');
