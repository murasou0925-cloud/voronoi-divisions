...
disp(['secpoint=', num2str(secpoint)]);
...
% 判定したい点 (x, y)
point = [Param.stepgoal(1), Param.stepgoal(2)];  % x, y は判定したい点の座標
...
% secpointがゴールと一致していればバッファは不要，それ以外ならバッファをとる
% if norm(Param.goal(1, :) - secpoint) > Param.th || (norm(Param.stepgoal(1,:) - secpoint(n,:)) > Param.subth)
%     secpoint = secpoint + vec1 * Param.buffer;
% end
